...
def ext_link(label, href):
    return A(label, href=href, target="_blank", rel="noopener", cls="underline decoration-slate-400/50 underline-offset-4 hover:decoration-current")
...
def summary_view(row):
    ...
    links.append(A("Raw markdown ↗", href=f"/item.md?id={row['hn_id']}", target="_blank", rel="noopener", cls="uk-btn uk-btn-default"))
    return Div(
...
app, rt = fast_app(hdrs=(THEME_BOOT, Link(rel='icon', type='image/svg+xml', href='/static/favicon.svg'), Link(rel='shortcut icon', href='/static/favicon.svg'), *Theme.slate.headers(), THEME_JS, CUSTOM_CSS), title=APP_TITLE, live=False)
