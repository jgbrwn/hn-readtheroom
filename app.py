...
def stat(label, value): return Div(P(label, cls="text-xs uppercase tracking-widest text-slate-500"), P(value if value is not None else "—", cls="text-lg font-medium"), cls="rounded-xl border border-slate-200 bg-white/70 p-4")
...
def summary_view(row):
    ...
    return Div(
        ...
        Article(ArticleTitle(A(row['title'], href=row['hn_url'], target="_blank", rel="noopener", cls="hover:underline")), ArticleMeta(f"generated by {row['model']} on {nice_date(row['generated_at'])} · prompt {row['prompt_version']}"), Div(*meta, cls="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-6 mb-6"), Div(*links, cls="flex flex-wrap gap-3 mb-8"), Div(render_md(row['markdown']), cls="mt-8 prose prose-slate max-w-none leading-8")))
...
def get():
    return page(
        ...
        Div(stat("cache", "SQLite"), stat("models", "free 1M+ context"), stat("input", "strictly validated"), cls="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-6"))
